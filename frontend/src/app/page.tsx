...
export default function LandingPage() {
  return (
    <div className="w-full">
      {/* Cosmic 3D hero intro (components/ui/horizon-hero-section.tsx) */}
      <HorizonHero />

      {/* Rest of the page: opaque + positioned so it fully occludes the hero's
          fixed canvas behind it (no bleed-through in the gaps between sections) */}
      <div className="relative z-10 bg-background">
        {/* Ambient background for the rest of the page */}
        <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
          <div className="absolute -top-32 left-1/2 -translate-x-1/2 h-[32rem] w-[60rem] rounded-full bg-indigo-500/10 blur-[120px]" />
          <div className="absolute top-1/3 -left-40 h-96 w-96 rounded-full bg-violet-500/10 blur-[100px]" />
          <div className="absolute bottom-0 -right-40 h-96 w-96 rounded-full bg-pink-500/10 blur-[100px]" />
          <div className="absolute inset-x-0 top-0 h-[36rem] bg-grid-fade" />
        </div>

        {/* Hero */}
        <section className="relative z-10 bg-background max-w-5xl mx-auto text-center pt-10 sm:pt-16 pb-14 px-2">
        ...
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3 animate-fade-up [animation-delay:180ms]">
          <Link
            ...
            className="group inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:scale-[0.98] text-white text-sm font-semibold shadow-lg shadow-indigo-600/25 hover:shadow-xl hover:shadow-indigo-600/30 transition-all cursor-pointer"
          >
      ...
      <section className="max-w-6xl mx-auto px-2 pb-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {PIPELINE_STEPS.map((step, idx) => (
            <div
              key={step.title}
              className="relative rounded-2xl border border-border bg-card p-5 shadow-sm hover:border-indigo-500/30 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 group animate-rise-in"
      ...
      <section className="max-w-3xl mx-auto px-2 pb-20 text-center">
        <div className="rounded-2xl border border-border bg-gradient-to-br from-indigo-600 to-violet-600 p-8 sm:p-10 shadow-xl">
          <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
            Ask it what to buy.
          </h2>
          <p className="text-sm text-indigo-100 mt-2 max-w-md mx-auto">
            Open the interactive chat and describe what you need — budget, use-case, dealbreakers.
          </p>
          <Link
            href="/advisor?mode=chat"
            className="group inline-flex items-center gap-2 mt-6 px-6 py-3 rounded-xl bg-white hover:bg-indigo-50 text-indigo-700 text-sm font-bold shadow-lg transition-all cursor-pointer"
          >
            <span>Try now</span>
            <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-0.5" />
          </Link>
