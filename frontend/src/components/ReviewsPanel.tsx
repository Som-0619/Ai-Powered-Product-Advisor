    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-surface-50 border border-border rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl p-6 relative transition-colors duration-200">
        ...
        <div className="flex items-center space-x-3 mb-5 border-b border-border pb-4">
          ...
          <div>
            <h3 className="text-base font-bold text-foreground tracking-tight">Grounded Review Analysis & Trust Audit</h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-5">
          <div className="bg-surface-100 p-3 rounded-xl border border-border">
            <span className="text-[11px] text-muted-foreground block uppercase font-semibold">Sentiment</span>
          ...
          <div className="bg-surface-100 p-3 rounded-xl border border-border col-span-2 sm:col-span-1">
            <span className="text-[11px] text-muted-foreground block uppercase font-semibold">Signals Analyzed</span>
          ...
          <div className="mb-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
            ...
            <div className="space-y-2">
              {reviews.suspicious_signals.map((sig, i) => (
                <div key={i} className="bg-surface-50 p-3 rounded-xl border border-border/70">
                  <div className="flex items-center justify-between text-xs font-semibold mb-1">
                    <span className="text-foreground uppercase tracking-wider">{sig.signal_type}</span>
                    <span className="text-[10px] text-muted-foreground">{sig.review_ids.length} affected reviews</span>
                  </div>
                  <p className="text-xs text-foreground">{sig.explanation}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-surface-100/50 p-4 rounded-xl border border-border space-y-2">
