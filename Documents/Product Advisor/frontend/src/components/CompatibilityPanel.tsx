    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-surface-50 border border-slate-200/80 dark:border-white/10 rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-y-auto shadow-2xl p-6 relative transition-colors duration-200">
        ...
        <div className="flex items-center space-x-3 mb-5 border-b border-slate-200/80 dark:border-white/5 pb-4">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-white tracking-tight">Electrical & Protocol Compatibility</h3>
        ...
        {comp?.checks && comp.checks.length > 0 ? (
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {comp.checks.map((c, i) => (
                <div key={i} className="bg-surface-100/50 p-3.5 rounded-xl border border-slate-200/80 dark:border-white/5 space-y-1">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-300">{c.reasoning}</p>
                </div>
              ))}
            </div>
          </div>
        ) : (
          <div className="bg-surface-100/50 p-4 rounded-xl border border-slate-200/80 dark:border-white/5 space-y-2 text-xs text-slate-600 dark:text-slate-300">
            <p>
